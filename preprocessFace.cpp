...
Mat processDetectedFace(Mat &faceImg , int desiredFaceWidth , CascadeClassifier &eyeCascade1, CascadeClassifier &eyeCascade2, bool doLeftAndRightSeparately)
{
        // Use square faces.
        ...
        if (leftEye.x >= 0 && rightEye.x >= 0)
        {
            ...
            return dstImg; //return processed image
        }
    return Mat();
}

//new version for processing face using nose detection instead of eyes detection
Mat processDetectedFace_(Mat &faceImg , int desiredFaceWidth , CascadeClassifier &noseCascade1)
{
        // Use square faces.
        int desiredFaceHeight = desiredFaceWidth;
        ...
        // Search for the 2 eyes at the full resolution, since eye detection needs max resolution possible!
        vector<Rect> rectNoses;
        noseCascade1.detectMultiScale( gray, rectNoses, 1.1, 2, 0|CV_HAAR_SCALE_IMAGE, Size(20, 20) );

        if(rectNoses.size()== 1) //if one nose detected
        {
           ...
           equalizeHist(gray, gray);
            ...
            // Filter out the corners of the face, since we mainly just care about the middle parts.
            // Draw a filled ellipse in the middle of the face-sized image.
            Mat mask = Mat(gray.size(), CV_8U, Scalar(0)); // Start with an empty mask.
            Point faceCenter = Point( desiredFaceWidth/2, cvRound(desiredFaceHeight * FACE_ELLIPSE_CY) );
            Size size = Size( cvRound(desiredFaceWidth * FACE_ELLIPSE_W), cvRound(desiredFaceHeight * FACE_ELLIPSE_H) );
            ellipse(mask, faceCenter, size, 0, 0, 360, Scalar(255), CV_FILLED);

            // Use the mask, to remove outside pixels.
            Mat dstImg = Mat(gray.size(), CV_8U, Scalar(128)); // Clear the output image to a default gray.
